...
| `cookbooks.html` + `cookbooks/catalog.json` | everyone | **~56 [Claude Cookbooks](https://github.com/anthropics/claude-cookbooks) (Anthropic, MIT)** curated and grouped by CCDV-F domain / bootcamp day. Each card opens the notebook **rendered via nbviewer** (no install) or on GitHub. Six recipes are distilled into runnable `../code-snippets/` (`workflow_patterns.py`, `orchestrator_workers.py`, `evaluator_optimizer.py`, `cookbook_building_evals.py`, `usage_cost_api.py`, `extended_thinking.py`). Slugs (`CB:<slug>`) are reference-chip targets in the decks (`data-refs`) and the exam runner (`refs:`). |
